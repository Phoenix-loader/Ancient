...
Ancient.__index.RgbToHexText = function(self, colors, text)
...
Ancient.__index.HandleReference = function(self, state)
...
Ancient.__index.MultiCallBack = function(self, tab, fn, call_init)
	if call_init then
		fn()
	end
...
Ancient.__index.GetIcon = function(self, name, type, color)
...
Ancient.__index.CreateAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
...
Ancient.__index.ProxyBind = (function()
...
Ancient.__index.CreateCHelpers = function(self)
...
Ancient.__index.GetCvar = function(self)
...
Ancient.__index.CreateRelativeAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
	local decode = self:UTF8DecodeSupport(text)
	if decode.length <= 0 then
...
Ancient.__index.NewEntity = function(self, ent)
	return setmetatable({
		ent = ent,
...
Ancient.__index.GetTickcountSwitch = function(self, ticks)
	return self.data.UpdateTickcountState[ticks]
end

Ancient.__index.Vector2DValid = function(self, vector, radius)
	return vector and vector.x ~= 0 and vector.y ~= 0 and not self:OffScreen(vector, radius)
end

Ancient.__index.OffScreen = function(self, position, radius)
...
Ancient.__index.UpdateUseKey = function(self, state)
...
Ancient.__index.MicroMove = function(self, cmd)
...
Ancient.__index.ExtrapolatePosition = function(self, player, origin, ticks)
...
Ancient.__index.UpdateTickcount = function(self, tickcount)
	for index = 2, 128 do
...
Ancient.__index.CalculateYaw = function(self, local_angle, enemy_angle)
...
Ancient.__index.GetLerpAnimation = function(self, key, set_var)
...
Ancient.__index.GetStaticTargetAnimation = function(self, key, set_var)
...
Ancient.__index.GetTimerSwitch = function(self, timer)
...
Ancient.__index.NewLerp = function(self, current, target, animation, key)
	if not self.data.CachedLerp[key] then
...
Ancient.__index.GetTimerSwitchWays = function(self, timers, data)
	local CachedKey = ""
...
Ancient.__index.CreateStaticResetAnimation = function(self, min, max, condition, animation, key)
	local math_vars = self:GetMathVars(min, max)
...
Ancient.__index.CreateStaticTargetAnimation = function(self, current, target, animation, key)
	if not self.data.CachedStaticSingleAnimation[key] then
...
Ancient.__index.Update = function(self, min, max, between, key)
...
Ancient.__index.Cycle = function(self, min, max, between, key)
	local between = math.abs(between)
	local is_reserved = min < 0 and max < 0
	local math_vars = self:GetMathVars(min, max)
...
Ancient.__index.CreateWays = function(self, ways, angles)
...
Ancient.__index.AutomaticSaveConfig = function(self)
...
Ancient.__index.ExportConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	self.data.Clipboard.set(self.data.Base64.encode(json.stringify(ConfigPackage)))
...
Ancient.__index.ImportConfig = function(self)
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	local Configs = self.data.Clipboard.get()
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	if not Configs then
		client.error_log(("[%s] Config Failed Import: The Clipboard Data Is Ancient Data"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
	...
	local Successed, JsonData = pcall(self.data.Base64.decode, Configs)
	if not Successed or not JsonData then
		client.error_log(("[%s] Config Failed Import: The Config Is Ancient Config, Please Check Your Clipboard"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
Ancient.__index.DeleteConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	...
	if not self.data.ConfigsList[ConfigName] then
		client.error_log(("[%s] Config Failed Delete: This Config Is Ancient"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
Ancient.__index.CreateConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
...
Ancient.__index.SaveConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	local ConfigIndex = self.Elements.Configs.ConfigsList:get()
	if not ConfigIndex then
	...
	if not ConfigName then
		client.error_log(("[%s] Config Failed Save: You Cant Save a Ancient Config"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			UserName = ("User: %s"):format(self.data.SelfName),
			AnimationColor = self.data.NotifyWarningAnimationColor,
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Save a Ancient Config")),
			Title = ("Ancient%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
...
Ancient.__index.LoadConfig = function(self, base_config)
	local Configs = {}
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	...
	if not BaseConfigs or type(BaseConfigs) ~= "string" then
		if not self.data.ConfigsList[ConfigName] then
			client.error_log(("[%s] Config Failed Load: You Cant Load a Ancient Config"):format(self.data.ScriptName))
			table.insert(self.data.NotifyCached, {
				Switch = false,
				CurrentIndex = false,
				Timer = self:GetTimeScale(),
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Ancient Config")),
				Title = ("Ancient%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
	...
	if not Successed or not JsonData then
		if type(base_config) ~= "string" then
			...
			table.insert(self.data.NotifyCached, {
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Ancient Data Config")),
				Title = ("Ancient%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
...
Ancient.__index.GetPlayerState = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
		return
	end
...
Ancient.__index.ManualAntiAim = function(self)
...
Ancient.__index.CanAttack = function(self, player, only_fire, adder)
...
Ancient.__index.RectangleOutline = function(self, x, y, w, h, round, thickness, color, settings, round_percentage)
...
Ancient.__index.RenderRoundRectangle = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderLeftOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderRightOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderBackground = function(self, x, y, w, h, color)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderGlowText = function(self, x, y, color, flags, max_width, text, glow_radius, ref_text)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderGlowOutline = function(self, x, y, w, h, width, rounding, color, settings, round_percentage)
	if color[4] <= 0 then
		return
	end
...
Ancient.__index.RenderGlowOutlineRectangle = function(self, x, y, w, h, rounding, glow_thickness, colors, settings, round_percentage)
	if settings then
...
Ancient.__index.TraceClosestPointRay = function(self, ray_from, ray_to, desired_point)
...
Ancient.__index.UpdateHistory = function(self, position, ticks, key)
...
Ancient.__index.CalculateFreestanding = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
...
Ancient.__index.Render3DCircle = function(self, x, y, z, radius, colors, accuracy, width, outline, start_degrees, percentage, removed_radius, style)
...
Ancient.__index.ShouldAntiAim = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
		return false
	end
...
Ancient.__index.UpdateDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
...
Ancient.__index.UpdateTickbaseDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
...
Ancient.__index.NetUpdateStart = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Ancient.__index.NetUpdateEnd = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Ancient.__index.FastLadder = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
...
Ancient.__index.PlayerHurt = function(self, e)
...
Ancient.__index.BulletImpact = function(self, e)
...
Ancient.__index.AimHit = function(self, e)
...
Ancient.__index.AimMiss = function(self, e)
	if not e.target or not self.Elements.Visuals.HitlogIndication:get() or not self.Elements.MasterSwitch:get() then
		return
	end
...
Ancient.__index.PreRender = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
...
Ancient.__index.Render = function(self)
	local tickcount = globals.tickcount()
...
--last part
	...
	if DebugFraction > 0 then
    	...
    	--------wide height
		local NameTextSize = self.data.Vector(renderer.measure_text("Ancient.\aA8A5F1FFdebug"))
		local GlowThickness = (100 / 10) or 10
		local GlowFraction = 100 or 70
		...
		self:RgbaToHexGradientText(DebugPanelColor, DebugPanelColor, "Ancient.\aA8A5F1FFdebug")
		renderer.text(x + 45, y - 5, 220, 220, 220, 255 * DebugFraction, "c-", 0, "Ancient.\aA8A5F1FFdebug        R")
	...
	if CrosshairFraction > 0 then
		...
		local PlayerStateTextSize = self.data.Vector(renderer.measure_text(CrosshairStyle == "Manifold" and "c-" or "c-", CrosshairStyle == "Manifold" and ("Ancient / %s"):format(("%i"):format(VelocityModifier * 100), "%") or self.data.PlayerStateName:upper()))
...
Ancient.__index.TickBase = function(self, e)
	local FakeDuck = ui.get(self.References.fake_duck)
	local local_player = self:NewEntity(entity.get_local_player())
	local LagCompensation = self:GetCvar("cl_lagcompensation")
...
Ancient.__index.FakeLag = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
...
Ancient.__index.AntiAim = function(self, e)
...
Ancient.__index.PaintUI = function(self)
	self:Render()
end

Ancient.__index.SetupCommand = function(self, e)
	self:AntiAim(e)
	self:FakeLag(e)
	self:TickBase(e)
	self:FastLadder(e)
...
Ancient.__index.PostConfigLoad = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Ancient.__index.PostConfigSave = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
	...
	if AutomaticSaveConfig then
		local ConfigPackage = self:GetPackages(self.Elements)
		database.write("[Ancient.debug]Configs", self.data.Configs)
		database.write("[Ancient.debug]Configs Data", self.data.ConfigsList)
		self.data.ConfigsList[self.data.AutoSaveConfigName] = self.data.Base64.encode(json.stringify(ConfigPackage))
	end
...
Ancient.__index.ShutDown = function(self)
	self:UpdateUseKey(true)
	...
	local local_player = self:NewEntity(entity.get_local_player())
	database.write("[Ancient.debug]Configs Data", self.data.ConfigsList)
	database.write("[Ancient.debug]Fired Last Update Timer", self.data.LastUpdateTimer)
	database.write("[Ancient.debug]Base Config", self.data.Base64.encode(json.stringify(ConfigPackage)))
	for index, material in pairs(self.data.ZoomMaterials) do
		if material ~= nil then
			material:reload()
		end
...
Ancient.__index.CallBacks = function(self)
	return {
		["aim_hit"] = function(e)
			self:AimHit(e)
		end,
...
Ancient.__index.Work = function(self)
	jit.on()
	self:CreateCHelpers()
	self:CreateElements()
	self:FiredUpdateLog()
